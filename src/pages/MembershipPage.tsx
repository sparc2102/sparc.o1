...
// Define animation variants with explicit TypeScript type
...
export function MembershipPage() {
  ...
  // Hooks for in-view animations
  const [tiersRef, tiersInView] = useInView({ triggerOnce: true, threshold: 0.2 });
  const [compareRef, compareInView] = useInView({ triggerOnce: true, threshold: 0.2 });
    ...
    <div className="min-h-screen bg-black">
      ...
      >
        ...
        <div className="max-w-5xl mx-auto">
          <motion.div
            variants={childVariants}
            className="bg-gray-900 bg-opacity-20 backdrop-blur-md rounded-lg shadow-lg overflow-hidden border border-gray-200 border-opacity-20"
          >
        ...
        >
          {faqs.map((faq, index) => (
            <motion.div
              variants={childVariants}
              key={index}
              className="border border-gray-200 rounded-xl shadow-sm bg-white bg-opacity-10 backdrop-blur-md p-4 transition-all duration-300 hover:shadow-lg hover:-translate-y-2"
            >
              <button
